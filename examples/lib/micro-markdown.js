...
var micromarkdown = {
  ...
  parse: function (str, strict) {
    ...
    /* olists */
    while ((stra = micromarkdown.regexobject.olists.exec(str)) !== null) {
      casca = 0;
    ...
    /* ulists */
    while ((stra = micromarkdown.regexobject.ulists.exec(str)) !== null) {
      casca = 0;
      if ((stra[0].trim().substr(0, 1) === '*') || (stra[0].trim().substr(0, 1) === '-')) {
        repstr = '<ul>';
      } else {
    ...
    /* underline */
    for (i = 0; i < 3; i++) {
      while ((stra = micromarkdown.regexobject.underline.exec(str)) !== null) {
        repstr = [];
        if (stra[1] === '~~') {
          str = str.replace(stra[0], '<del>' + stra[2] + '</del>');
        } else {
